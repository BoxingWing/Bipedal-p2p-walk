...
## Modification by me!
disable LFS settings for usd files
remove git state logging function in train.py
...
- Throughout the repository, the name `AzureLoong` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of AzureLoong (in files/directories) to your_fancy_extension_name
...
We provide an example UI extension that will load upon enabling your extension defined in `source/AzureLoong/AzureLoong/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/AzureLoong"
    ]
}
```
...
{
    "python.analysis.extraPaths": [
        "${workspaceFolder}/source/AzureLoong",
