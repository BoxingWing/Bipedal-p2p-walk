...
import torch
import numpy as np
from cmd_generators import cmd_scheduler, cmd_scheduler_tensor, euler_expander
...
from isaaclab.utils.math import quat_from_angle_axis, yaw_quat, quat_apply_yaw
    ...
    marker_cfg = VisualizationMarkersCfg(
        ...
        markers={
            ...
            "sphere": sim_utils.SphereCfg(
                radius=0.025,
                visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.133, 0.545, 0.133)),
            ),
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, policy, env_cfg, sim_cfg):
    ...
    euler_cal = euler_expander(num_envs=scene.num_envs, device=sim.device)
    ...
    ang_vel_yaw = [-0.3, -0.1, 0.1, 0.3] # min max min max [rad/s]
    ...
    px = torch.ones_like(vx_rand * period)*1.2
    py = torch.ones_like(vy_rand * period)*0. 
    thetaZ = torch.ones_like(px) * 0. #wz_rand * period 
    ...
    vy_rand = torch_rand_float_ranges(lin_vel_y[0], lin_vel_y[1],lin_vel_y[2],lin_vel_y[3],(scene.num_envs, 1), device=sim.device)
    wz_rand = torch_rand_float_ranges(ang_vel_yaw[0], ang_vel_yaw[1],ang_vel_yaw[2],ang_vel_yaw[3],(scene.num_envs, 1), device=sim.device)
    px = torch.ones_like(vx_rand * period)*0.5
    py = torch.ones_like((vy_rand * period))*0. 
    thetaZ = torch.ones_like(wz_rand * period)*0. 
    isStand = torch.full((scene.num_envs, 1), False, dtype=torch.bool, device=sim.device)
    cmd.add_mode(period=period,px=px,py=py,thetaZ=thetaZ,isStand=isStand)
    ...
    vy_rand = torch_rand_float_ranges(lin_vel_y[0], lin_vel_y[1],lin_vel_y[2],lin_vel_y[3],(scene.num_envs, 1), device=sim.device)
    wz_rand = torch_rand_float_ranges(ang_vel_yaw[0], ang_vel_yaw[1],ang_vel_yaw[2],ang_vel_yaw[3],(scene.num_envs, 1), device=sim.device)
    px = torch.ones_like(vx_rand * period) * 0.
    py = torch.ones_like(vy_rand * period) * 0.
    thetaZ = torch.ones_like(wz_rand * period) * 1. 
    isStand = torch.full((scene.num_envs, 1), False, dtype=torch.bool, device=sim.device)
    cmd.add_mode(period=period,px=px,py=py,thetaZ=thetaZ,isStand=isStand)
    ...
    cmd_count = torch.zeros((scene.num_envs), device=sim.device,dtype=torch.float) - 1.0
    cmd_update_flag = torch.zeros((scene.num_envs), device=sim.device, dtype=torch.bool)
    target_base_pos = torch.zeros((scene.num_envs,2), device=sim.device,dtype=torch.float)
    target_base_yaw = torch.zeros((scene.num_envs,1), device=sim.device,dtype=torch.float)
    start_base_pos = torch.zeros((scene.num_envs,2), device=sim.device,dtype=torch.float)
    start_base_yaw = torch.zeros((scene.num_envs,1), device=sim.device,dtype=torch.float)
    start_base_quat = torch.zeros((scene.num_envs,4), device=sim.device,dtype=torch.float)
    commands_w = torch.zeros((scene.num_envs,3), device=sim.device)
    # create markers
    my_visualizer = define_markers()
    yaw = torch.ones((4,1), device=sim.device) * 0.
    marker_locations = torch.zeros((4,3), device=sim.device)
    marker_orientations = quat_from_angle_axis(yaw, torch.tensor([0.0, 0.0, 1.0], device=sim.device)).squeeze(1)
    marker_indices = torch.tensor([0,1,0,2], device=sim.device)
    ...
    while simulation_app.is_running():
        ...
        if count>start_ctr:
            cmd.step()
            tmp = cmd.cmd_count
            cmd_update_flag = tmp > cmd_count
            cmd_count = cmd.cmd_count.clone()
        ...
        # base_euler_xyz = torch.stack((r, p, w), dim=1)
        # base_euler_xyz[base_euler_xyz > torch.pi] -= 2 * torch.pi
        euler_cal.cal(base_quat)
        base_euler_xyz = euler_cal.euler_xyz_multiloop.clone()
        ...
        start_base_pos[cmd_update_flag,:2] = root_states[cmd_update_flag,:2]
        start_base_yaw[cmd_update_flag,0] = base_euler_xyz[cmd_update_flag,2]
        start_base_quat[cmd_update_flag,:4] = root_states[cmd_update_flag,3:7]
        
        commands_w[:,:2] = cmd.cmd_cur[:,:2]
        commands_w = quat_apply_yaw(start_base_quat, commands_w)
        target_base_pos = start_base_pos + commands_w[:,:2]
        target_base_yaw = start_base_yaw + cmd.cmd_cur[:,2]
        # print("cmd_count ", cmd_count)
        # print("cmd_update_flag", cmd_update_flag)
        # print("cmd_cur", cmd.cmd_cur)
        # print("target_pos ", target_base_pos)
        # print("target_yaw ", target_base_yaw)
        ...
        if count % env_cfg.decimation == 0 :
            ...
            cmd_cur_tensor[:,0] = cmd.cmd_cur[:,0] * env_cfg.normalization.obs_scales.pos_xy
            cmd_cur_tensor[:,1] = cmd.cmd_cur[:,1] * env_cfg.normalization.obs_scales.pos_xy
            cmd_cur_tensor[:,2] = cmd.cmd_cur[:,2] * env_cfg.normalization.obs_scales.pos_theta
            cmd_next_tensor[:,0] = cmd.cmd_next[:,0] * env_cfg.normalization.obs_scales.pos_xy
            cmd_next_tensor[:,1] = cmd.cmd_next[:,1] * env_cfg.normalization.obs_scales.pos_xy
            cmd_next_tensor[:,2] = cmd.cmd_next[:,2] * env_cfg.normalization.obs_scales.pos_theta
            print(cmd_cur_tensor)
            print('+', cmd_next_tensor)
            # print(cmd.phi)
        ...
        scene.update(sim_dt)
        # update markers
        marker_locations[0,:2] = target_base_pos[0,:2]
        marker_locations[1,:2] = target_base_pos[0,:2]
        marker_locations[2,:2] = robot.data.body_link_state_w[0, 0, :2]
        marker_locations[3,:2] = robot.data.body_link_state_w[0, 0, :2]
        base_yaw_quat = yaw_quat(robot.data.body_link_state_w[0, 0, 3:7])
        marker_orientations[2,:] = base_yaw_quat
        marker_orientations[3,:] = base_yaw_quat
        tmpquat = quat_from_angle_axis(target_base_yaw[0,0], torch.tensor([0.,0.,1.], device=sim.device))
        target_quat = quat_mul(robot.data.body_link_state_w[0, 0, 3:7], tmpquat)
        marker_orientations[0,:] = yaw_quat(target_quat)
        marker_orientations[1,:] = yaw_quat(target_quat)
        # record variables
...
def main():
    ...
    # design scene
    scene_cfg = RobotGymSceneCfg(num_envs=1, env_spacing=2.0)
